...
from promptly.domain_prompt.prompts.optimizer import (
    ANSWER_JUDGE_SYSTEM,
    MUTATION_SYSTEM,
    MUTATION_TIPS,
...
from promptly.llm.optimizer import build_duel_answerer, build_duel_judge, build_variant_generator
...
async def _duel(
    ...
    ans_a / ans_b: the answers generated by prompt_a and prompt_b respectively.
    ...
    ans_a, ans_b = (ans_second, ans_first) if flip else (ans_first, ans_second)
    ...
    try:
        ...
        return winner_original, gamma, ans_a, ans_b
    except Exception as exc:  # noqa: BLE001
        _log.warning("duel_judge_failed", error=str(exc))
        return rng.randint(0, 1), GAMMA_REASONING, ans_a, ans_b  # noqa: S311

"""add redis_job_id to pb_transfer_jobs

Revision ID: 4a36d01781da
Revises: 6e0e77f64d57
Create Date: 2026-05-13 21:33:00.827401
...
revision: str = "4a36d01781da"
down_revision: str | Sequence[str] | None = "6e0e77f64d57"
...
def upgrade() -> None:
    ...
    op.add_column(
        "pb_transfer_jobs", sa.Column("redis_job_id", sa.String(length=64), nullable=True)
    )
    op.create_index(
        op.f("ix_pb_transfer_jobs_redis_job_id"), "pb_transfer_jobs", ["redis_job_id"], unique=False
    )
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f("ix_pb_transfer_jobs_redis_job_id"), table_name="pb_transfer_jobs")
    op.drop_column("pb_transfer_jobs", "redis_job_id")
    op.create_table(
        "checkpoint_blobs",
